 ...
 * - Food: FAO/FOLU India Food Systems (2021), adapted for Indian dietary context
 * - Appliances: BEE (Bureau of Energy Efficiency) India
 */

// ─────────────────────────────────────────────
// ELECTRICITY: State-wise grid emission factors
// kg CO2 per kWh consumed (CEA v18, 2023)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// TRANSPORT: India-specific factors
// kg CO2 per km (lifecycle including fuel production)
// ─────────────────────────────────────────────
export const TRANSPORT_EMISSION_FACTORS = {
  petrol_car_small: 0.148,
  petrol_car_mid: 0.192,
  petrol_car_suv: 0.261,
  diesel_car: 0.171,
  cng_car: 0.098,
  electric_car: 0.065,
  two_wheeler_petrol: 0.063,
  two_wheeler_electric: 0.022,
  metro_delhi: 0.031,
  metro_bangalore: 0.038,
  metro_mumbai: 0.041,
  metro_hyderabad: 0.044,
  metro_chennai: 0.039,
  city_bus_diesel: 0.089,
  city_bus_electric: 0.031,
  auto_cng: 0.072,
  auto_petrol: 0.094,
  e_rickshaw: 0.028,
  indian_railways: 0.014,
    ...
    case "petrol_car":
      return engineSize === "small"
        ...
        : engineSize === "suv"
          ...
          : 0.192;
    case "diesel_car":
      return TRANSPORT_EMISSION_FACTORS.diesel_car;
    case "cng_car":
      return TRANSPORT_EMISSION_FACTORS.cng_car;
    case "electric_car":
      return TRANSPORT_EMISSION_FACTORS.electric_car;
    case "two_wheeler":
      return TRANSPORT_EMISSION_FACTORS.two_wheeler_petrol;
    case "e_scooter":
      return TRANSPORT_EMISSION_FACTORS.two_wheeler_electric;
    case "none":
      return 0;
    default:
      return TRANSPORT_EMISSION_FACTORS.default_car;
  }
}
...
// ─────────────────────────────────────────────
// FOOD: India-calibrated factors
// ─────────────────────────────────────────────
export const FOOD_EMISSION_FACTORS = {
  DIET_ANNUAL: {
    vegan_local: 800,
    vegetarian_dairy: 1400,
    eggetarian: 1650,
    non_veg_low: 1900,
    non_veg_moderate: 2400,
    non_veg_high: 3200,
  },
};

// ─────────────────────────────────────────────
// APPLIANCES: BEE India data
// ─────────────────────────────────────────────
export const APPLIANCE_FACTORS = {
  ac_1_ton_per_hour: 1.1,
  ac_15_ton_per_hour: 1.4,
  ac_2_ton_per_hour: 1.8,
  ac_default_per_hour: 1.4,
  ELECTRICITY_ANNUAL_KWH: {
    low: 900,
    medium: 1800,
    high: 3600,
    very_high: 6000,
...
export function calculateFootprint(answers: CalculatorAnswers): FootprintBreakdown {
  ...
  const transport =
    carAnnual + twoWheelerAnnual + publicTransportAnnual + flightAnnual;
